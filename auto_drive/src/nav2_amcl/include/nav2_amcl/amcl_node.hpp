...
protected:
  ...
  std::unique_ptr<message_filters::Subscriber<sensor_msgs::msg::LaserScan,
      ...
      laser_scan_sub_;
  std::unique_ptr<tf2_ros::MessageFilter<sensor_msgs::msg::LaserScan>>
      laser_scan_filter_;
   ...
   */
  bool checkElapsedTime(std::chrono::seconds check_interval,
   ...
   */
  bool
  updateFilter(const int &laser_index,
               const sensor_msgs::msg::LaserScan::ConstSharedPtr &laser_scan,
   ...
   */
  bool getMaxWeightHyp(std::vector<amcl_hyp_t> &hyps,
                       amcl_hyp_t &max_weight_hyps, int &max_weight_hyp);
  /*
   * @brief Publish robot pose in map frame from AMCL
   */
  void
  publishAmclPose(const sensor_msgs::msg::LaserScan::ConstSharedPtr &laser_scan,
