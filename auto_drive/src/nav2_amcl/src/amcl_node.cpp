...
AmclNode::AmclNode(const rclcpp::NodeOptions &options)
  ...
  add_parameter("robot_model_type",
...
AmclNode::on_activate(const rclcpp_lifecycle::State & /*state*/) {
  ...
  dyn_params_handler_ = node->add_on_set_parameters_callback(std::bind(
      &AmclNode::dynamicParametersCallback, this, std::placeholders::_1));
...
AmclNode::on_cleanup(const rclcpp_lifecycle::State & /*state*/) {
  ...
  if (set_initial_pose_) {
    ...
    set_parameter(rclcpp::Parameter(
        "initial_pose.yaw", rclcpp::ParameterValue(tf2::getYaw(
                                last_published_pose_.pose.pose.orientation))));
  }
...
bool AmclNode::checkElapsedTime(std::chrono::seconds check_interval,
  ...
  try {
    ...
    if (scan_error_count_ % 20 == 0) {
      RCLCPP_ERROR(get_logger(),
                   "(%d) consecutive laser scan transforms failed: (%s)",
                   scan_error_count_, e.what());
    }
    return false;
  }
...
void AmclNode::handleInitialPose(
  ...
  try {
    ...
    tx_odom = tf_buffer_->lookupTransform(
        base_frame_id_, tf2_ros::fromMsg(msg.header.stamp), base_frame_id_,
        tf2_time, odom_frame_id_);
  } catch (tf2::TransformException &e) {
    ...
    if (sent_first_transform_) {
      RCLCPP_WARN(get_logger(), "Failed to transform initial pose in time (%s)",
                  e.what());
    }
    tf2::impl::Converter<false, true>::convert(tf2::Transform::getIdentity(),
                                               tx_odom.transform);
  }
  ...
  RCLCPP_INFO(get_logger(), "Setting pose (%.6f): %.3f %.3f %.3f",
              now().nanoseconds() * 1e-9, pose_new.getOrigin().x(),
              pose_new.getOrigin().y(), tf2::getYaw(pose_new.getRotation()));
...
void AmclNode::laserReceived(
  ...
  std::string laser_scan_frame_id =
      nav2_util::strip_leading_slash(laser_scan->header.frame_id);
  ...
  if (frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end()) {
    if (!addNewScanner(laser_index, laser_scan, laser_scan_frame_id,
                       laser_pose)) {
...
bool AmclNode::updateFilter(
  ...
  RCLCPP_DEBUG(get_logger(),
               "Laser %d angles in base frame: min: %.3f inc: %.3f",
               laser_index, angle_min, angle_increment);

  // Apply range min/max thresholds, if the user supplied them
  if (laser_max_range_ > 0.0) {
    ldata.range_max =
        std::min(laser_scan->range_max, static_cast<float>(laser_max_range_));
  } else {
  ...
  if (laser_min_range_ > 0.0) {
    range_min =
        std::max(laser_scan->range_min, static_cast<float>(laser_min_range_));
  } else {
  ...
  lasers_[laser_index]->sensorUpdate(
      pf_, reinterpret_cast<nav2_amcl::LaserData *>(&ldata));
...
bool AmclNode::getMaxWeightHyp(std::vector<amcl_hyp_t> &hyps,
                               amcl_hyp_t &max_weight_hyps,
                               int &max_weight_hyp) {
  // Read out the current hypotheses
  double max_weight = 0.0;
  hyps.resize(pf_->sets[pf_->current_set].cluster_count);
  for (int hyp_count = 0; hyp_count < pf_->sets[pf_->current_set].cluster_count;
       hyp_count++) {
    double weight;
...
void AmclNode::publishAmclPose(
  ...
  p->pose.pose.orientation =
      orientationAroundZAxis(hyps[max_weight_hyp].pf_pose_mean.v[2]);
...
void AmclNode::sendMapToOdomTransform(
...
nav2_amcl::Laser *AmclNode::createLaserObject() {
  ...
  if (sensor_model_type_ == "beam") {
    return new nav2_amcl::BeamModel(z_hit_, z_short_, z_max_, z_rand_,
                                    sigma_hit_, lambda_short_, 0.0, max_beams_,
                                    map_);
  }
...
AmclNode::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
  std::lock_guard<std::recursive_mutex> cfl(mutex_);
...
void AmclNode::initMessageFilters() {
  ...
  laser_scan_sub_ = std::make_unique<message_filters::Subscriber<
...
void AmclNode::initPubSub() {
  ...
  initial_pose_sub_ =
      create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
          ...
          std::bind(&AmclNode::initialPoseReceived, this,
                    std::placeholders::_1));
