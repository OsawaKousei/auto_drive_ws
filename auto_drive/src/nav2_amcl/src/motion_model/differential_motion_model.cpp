...
void DifferentialMotionModel::initialize(double alpha1, double alpha2,
...
void DifferentialMotionModel::odometryUpdate(pf_t *pf, const pf_vector_t &pose,
  ...
  if (sqrt(delta.v[1] * delta.v[1] + delta.v[0] * delta.v[0]) < 0.01) {
    ...
    delta_rot1 =
        angleutils::angle_diff(atan2(delta.v[1], delta.v[0]), old_pose.v[2]);
  }
  ...
  for (int i = 0; i < set->sample_count; i++) {
    ...
    delta_trans_hat =
        delta_trans -
        pf_ran_gaussian(sqrt(alpha3_ * delta_trans * delta_trans +
...
PLUGINLIB_EXPORT_CLASS(nav2_amcl::DifferentialMotionModel,
