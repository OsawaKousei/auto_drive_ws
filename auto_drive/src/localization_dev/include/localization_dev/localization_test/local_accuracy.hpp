...
/// @details This class calculates the accuracy of the localization by comparing
/// the estimated odometry with the real odometry. The accuracy is calculated as
/// 1 / (1 + cbrt(dx^2 + dy^2 + dth^2)), where dx, dy, and dth are the
/// differences between the estimated and real odometry. The accuracy is
/// published as a Float64 message. The estimated odometry is received from the
/// /estimated_odom topic, and the real odometry is received from the /odom
/// topic.
