...
/// @details This class estimates the robot's pose by fusing the noisy odometry and the laser scan data.
/// The estimated pose is published as a Pose message. x,y are the position of the robot, and z is the orientation of the robot.
/// The noisy odometry is received from the /noisy_odom topic, the laser scan data is received from the /scan topic,
/// and the map is received from the /map topic and the point cloud is received from the /mapped_pc2 topic.
...
private:
  ...
  /// @details This function estimates the robot's pose by fusing the noisy odometry and the laser scan data.
  geometry_msgs::msg::Pose estimate_pose();

  /// @brief Callback function for the localization switch.
  /// @param msg
  /// @return void
  /// @details when the localization switch on simulator is pushed, this function will be called. 
