...
public:
  static void SetUpTestCase() {
    ...
    std::this_thread::sleep_for(
        std::chrono::seconds(5)); // allow node to start up
  ...
  static void TearDownTestCase() {
    ...
    lifecycle_client_->change_state(
...
std::shared_ptr<nav2_util::LifecycleServiceClient>
    MapServerTestFixture::lifecycle_client_ = nullptr;
...
TEST_F(MapServerTestFixture, NoInitialMap) {
  ...
  auto parameters_client =
      std::make_shared<rclcpp::SyncParametersClient>(node_, "map_server");
  ...
  RCLCPP_INFO(node_->get_logger(),
              "Removing yaml_filename-parameter before restarting");
  parameters_client->set_parameters(
      {Parameter("yaml_filename", ParameterValue(""))});
  ...
  RCLCPP_INFO(node_->get_logger(),
              "Testing LoadMap service while not being active");
  ...
  auto resp =
      send_request<nav2_msgs::srv::LoadMap>(node_, load_map_cl, load_map_req);

  ASSERT_EQ(resp->result,
            nav2_msgs::srv::LoadMap::Response::RESULT_UNDEFINED_FAILURE);
