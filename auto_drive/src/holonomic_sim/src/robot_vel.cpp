...
{
    publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("robot_vel", 10);
    ...
    auto topic_callback = [this](const nav_msgs::msg::Odometry &msg) -> void {
        ...
        rotation_matrix.setEulerYPR(-(yaw + prev_yaw)/2.0, 0, 0); //use the average to reduce error 
        diff_v = rotation_matrix * diff_v;

        //calculate the velocity
        double time_diff = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9 - prev_odom.header.stamp.sec - prev_odom.header.stamp.nanosec * 1e-9;
        ...
        if (robot_vel.linear.x != 0 || robot_vel.linear.y != 0 || robot_vel.angular.z != 0) {
            RCLCPP_INFO(this->get_logger(), "Robot velocity: x=%f, y=%f, z=%f", robot_vel.linear.x, robot_vel.linear.y, robot_vel.angular.z);
        }
