...
TEST(GeometryUtils, calculate_path_length) {
  ...
  while (current_polar_angle_deg != 360) {
    float x_loc =
        polar_distance * std::cos(current_polar_angle_deg * (pi / 180.0));
    float y_loc =
        polar_distance * std::sin(current_polar_angle_deg * (pi / 180.0));
  ...
  ASSERT_NEAR(calculate_path_length(circle_path), 2 * pi * polar_distance,
              1e-1);
}
