  ...
  preempted:
    ...
    for (int i = 1; (i < goal->order) && rclcpp::ok(); ++i) {
      ...
      if (action_server_->is_cancel_requested() ||
          !action_server_->is_server_active()) {
...
public:
  ...
  void Setup() {
    server_thread_ = std::make_shared<std::thread>(
        std::bind(&RclCppFixture::server_thread_func, this));
  }
...
public:
  ActionTestNode()
      : rclcpp::Node(
            nav2_util::generate_internal_node_name("action_test_node")) {}

  void on_init() {
    action_client_ = rclcpp_action::create_client<Fibonacci>(shared_from_this(),
                                                             "fibonacci");
    action_client_->wait_for_action_server();

    deactivate_pub_ =
        this->create_publisher<std_msgs::msg::Empty>("deactivate_server", 1);
    activate_pub_ =
        this->create_publisher<std_msgs::msg::Empty>("activate_server", 1);
    omit_prempt_pub_ =
        this->create_publisher<std_msgs::msg::Empty>("omit_preemption", 1);
  }
...
TEST_F(ActionTest, test_simple_action) {
  ...
  EXPECT_EQ(rclcpp::spin_until_future_complete(node_, future_goal_handle),
...
TEST_F(ActionTest, test_simple_action_with_feedback) {
  ...
  auto future_goal_handle =
      node_->action_client_->async_send_goal(goal, send_goal_options);
  EXPECT_EQ(rclcpp::spin_until_future_complete(node_, future_goal_handle),
            rclcpp::FutureReturnCode::SUCCESS);
...
TEST_F(ActionTest, test_simple_action_preemption_after_succeeded) {
  ...
  auto future_preempt_handle =
      node_->action_client_->async_send_goal(preemption);
  EXPECT_EQ(rclcpp::spin_until_future_complete(node_, future_goal_handle),
            rclcpp::FutureReturnCode::SUCCESS);
...
TEST_F(ActionTest, test_handle_cancel) {
  ...
  auto cancel_response =
      node_->action_client_->async_cancel_goal(future_goal_handle.get());
  EXPECT_EQ(rclcpp::spin_until_future_complete(node_, cancel_response),
            rclcpp::FutureReturnCode::SUCCESS);

  // Check cancelled
  EXPECT_EQ(future_goal_handle.get()->get_status(),
            rclcpp_action::GoalStatus::STATUS_CANCELING);
