...
bool getCurrentPose(geometry_msgs::msg::PoseStamped &global_pose,
                    ...
                    const double transform_timeout, const rclcpp::Time stamp) {
...
bool transformPoseInTargetFrame(
  ...
  static rclcpp::Logger logger =
      rclcpp::get_logger("transformPoseInTargetFrame");
  ...
  try {
    ...
    RCLCPP_ERROR(logger, "Transform timeout with tolerance: %.4f",
                 transform_timeout);
...
bool getTransform(const std::string &source_frame_id,
  ...
  try {
    ...
    transform =
        tf_buffer->lookupTransform(target_frame_id, source_frame_id,
